...
public class AirportGraph implements IAirportGraph, GraphADT<IAirport>{
    ...
    protected class Edge {
        ...
        public Edge(Vertex target, double weight, IFlight flight) {
        	this.target = target;
        	this.weight = weight;
        	this.flight = flight;
        }
    ...
    protected Hashtable<IAirport, Vertex> vertices; // holds graph verticies, key=data
    public AirportGraph() { vertices = new Hashtable<>(); }
     ...
     */
    public boolean insertEdge(IAirport source, IAirport target, double weight) {
    ...
    public boolean insertEdge(IAirport source, IAirport target, IFlight flight) {
    	if(source == null || target == null || flight == null) 
            throw new NullPointerException("Cannot add edge with null source, target or flight");
        Vertex sourceVertex = this.vertices.get(source);
        Vertex targetVertex = this.vertices.get(target);
        ...
        if(flight.getCost() < 0) 
            throw new IllegalArgumentException("Cannot add edge with negative cost");
        // handle cases where edge already exists between these verticies
        for(Edge e : sourceVertex.edgesLeaving)
        	if(e.target == targetVertex) {
        		if(e.weight == flight.getCost()) return false; // edge already exists
        		else e.weight = flight.getCost(); // otherwise update weight of existing edge
        		return true;
             }
        // otherwise add new edge to sourceVertex
        sourceVertex.edgesLeaving.add(new Edge(targetVertex, flight.getCost(), flight));
        return true;   	
    }
    ...
    public List<IFlight> cheapestFlight(IAirport start, IAirport end){
        return dijkstrasShortestPath(start, end).flightSequence;
    }
     ...
     */
    public double getWeight(IAirport source, IAirport target) {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public List<IAirport> dataSequence; // ordered sequence of data from vertices in path
        public List<IFlight> flightSequence; // ordered sequence of flights 
        ...
        public Path(Vertex start) {
            ...
            this.flightSequence = new LinkedList<>();
            this.end = start;
        }
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            for(int i = 0; i < copyPath.flightSequence.size(); i++) {
            	this.flightSequence.add(copyPath.flightSequence.get(i));
            }
            this.distance = copyPath.distance + extendBy.weight;
            this.dataSequence.add(extendBy.target.data);
            this.flightSequence.add(extendBy.flight);
        ...
        public int compareTo(Path other) {
            int cmp = (int)this.distance - (int)other.distance;
     ...
     */
    protected Path dijkstrasShortestPath(IAirport start, IAirport end) throws NoSuchElementException{
    	if(!this.vertices.containsKey(start) || !this.vertices.containsKey(end)) 
    		throw new NoSuchElementException("No path found with those vertices");
        Vertex Vstart = this.vertices.get(start);
        Path path1 = new Path(Vstart);
        PriorityQueue<Path> pqueue = new PriorityQueue<>(); //contains paths in queue
        List<Vertex> visited = new LinkedList<>(); //contains all visited vertices
        PriorityQueue<Path> complete = new PriorityQueue<>(); //contains all complete paths from start to end
        visited.add(Vstart);
        //adds the first few paths to pqueue
        for(int i = 0; i < Vstart.edgesLeaving.size(); i++)
        	pqueue.add(new Path(path1, Vstart.edgesLeaving.get(i)));
        //checks if the queued up path reaches the end, adds new paths
        while(pqueue.size() != 0 ) {
        	Path current = pqueue.poll();
        	if(current == null) break;
        	visited.add(current.end);
        	if(current.end.data.equals(end)) {
        		complete.add(current);
        		continue;
        	}
        	//checks if the new vertex has been visited, if not add new path
        	for(int j = 0; j < current.end.edgesLeaving.size(); j++) {
        		if(visited.contains(current.end.edgesLeaving.get(j).target)) continue;
        		pqueue.add(new Path(current, current.end.edgesLeaving.get(j)));
        	}
        }
        if(complete.size()==0) throw new NoSuchElementException("No path found with those vertices");
     ...
     *     including when no vertex containing start or end can be found
     */
    public List<IAirport> shortestPath(IAirport start, IAirport end) {
     ...
     *     including when no vertex containing start or end can be found
     */
    public double getPathCost(IAirport start, IAirport end) {
