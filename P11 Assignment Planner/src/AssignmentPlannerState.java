...
public enum AssignmentPlannerState {
	MAIN_MENU {
		...
		/**
		 * Prints possible options for actions to take to the user, and collects their
		 * input. If the user's input is not a valid action, a warning message should be
		 * printed, and the next state should also be MAIN_MENU.
		 *
		 * If option [1] Add an assignment is selected, the queue should not be
		 * modified, and the next state should be ADD_ASSIGNMENT.
		 *
		 * If option [2] Show highest priority assignment is selected, the earliest
		 * assignment in the queue should be printed, and the next state should be
		 * MAIN_MENU. If the queue is empty, a warning message should be printed.
		 *
		 * If option [3] Complete highest priority assignment is selected, the earliest
		 * assignment in the queue should be printed and removed, and the next state
		 * should be MAIN_MENU. If the queue is empty, a warning message should be
		 * printed.
		 *
		 * If option [4] List all assignments is selected, all assignments in the queue
		 * should be printed in order from earliest to latest, and the next state should
		 * be MAIN_MENU.
		 *
		 * If option [5] Clear planner is selected, all assignments should be removed
		 * from the queue, and the next state should be MAIN_MENU.
		 *
		 * If option [0] Quit planner is selected, a goodbye message should be printed
		 * to the user, and the next state should be DONE.
		 *
		...
		public AssignmentPlannerState runState(Scanner scanner, AssignmentQueue queue) {
			...
			if (num == 1) {
				...
				try {
					System.out.println("Highest priority assignment is:\n" + queue.peek().toString());
				} catch (Exception e) {
	...
	ADD_ASSIGNMENT {
		...
		/**
		 * Asks the user for the information of the assignment they would like to add,
		 * in the order name, month, day of month, and hour.
		 *
		 * If the queue is at capacity, a warning message should be printed, and the
		 * next state should be MAIN_MENU.
		 *
		 * If there is a problem with the user's input, no new Assignment should be
		 * added to the queue, and the next state should be ADD_ASSIGNMENT.
		 *
		 * If the user correctly inputs all fields, an assignment with the desired
		 * fields should be created and added to the queue, and the next state should be
		 * MAIN_MENU.
		 *
		 * @param scanner scanner for user input
		 * @param queue   current Assignment queue
	...
	/**
	 * Performs the behavior of this state by utilizing the values given by the
	 * scanner, updating the queue, and returning the next state
	 * 
	 * @param scanner scanner for user input
	 * @param queue   current Assignment queue
	 * @return the next state of the AssignmentPlanner
