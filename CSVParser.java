...
public class CSVParser {
    ...
    public static String[] separate(String text) {
        ...
        String str1="";
        int h=0;
        String[] str = new String[comma+1];
        if(text.charAt(text.length()-1)==',') {
      	   for(int j=0;j<text.length();j++) {
      		   if(text.charAt(j)!=',') {
      			   str1=str1+text.charAt(j);
      		   }
      		   else {
      			   str[h]=str1;
      			   str1="";
    ...
    public static String[] separate(String text, char delimiter) {
        ...
        String[] str=text.split(delimiter1);
        return str;
    ...
    public static String[] separate(String text, char delimiter, char qualifier) {
    	...
    	CSVParser parser = CSVFormat.EXCEL.withHeader("Col1", "Col2", "Col3").parse(text);
        ...
        int h=0;
    	String[] str = new String[comma+1];
        if(text.charAt(text.length()-1)==delimiter) {
      	   for(int j=0;j<text.length();j++) {
        	...
        	if(text.indexOf(qualifier)>0) {
        		String[] str2=new String[text.indexOf(qualifier)/2];
        		for(int j=0;j<text.length();j++) {
           		   if(text.charAt(j)!=',') {
           			   str1=str1+text.charAt(j);
           		   }
